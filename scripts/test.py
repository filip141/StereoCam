...
while (1):
    ...
    pixels_dif[pixels_dif < 0.0325 * disp.size] = 0
    ...
    for peak_ind, val in zip(peak_max_ind, peaks_max_val):
        ...
        tresh_temp = cv2.bitwise_and(disp_norm, disp_norm, mask=mask)
        mask_list.append(tresh_temp)
    # print frame_l.size
    diff_array = np.array([x[0] for x in point_list])
    #
    for object_mask in mask_list:
        # fig, ax = plt.subplots()
        # im = ax.imshow(object_mask, interpolation='none')
        # ax.format_coord = Formatter(im)
        # plt.show()
        object_mask = (object_mask * 2.50).astype(np.uint8)
        cnts = cv2.findContours(object_mask, cv2.RETR_LIST,
        ...
        if cnts:
            ...
            obj_distance = int(Q[2,3]*(1/Q[3,2])/disp[center]*1.0)
            cv2.putText(rect_frames[0], str(obj_distance), (x+w-30, y+h), cv2.FONT_HERSHEY_SIMPLEX,
                                0.5, (0 , 255, 0), 1, 1)
    ...
    mask_img = mask_list[0] * rect_frames[0]
    try:
        cv2.imshow('tr1', mask_img)
    except:
        pass
    try:
